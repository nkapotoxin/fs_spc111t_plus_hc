...
@require_admin_context
def network_get_allocated_fixed_ips(context, network_id, host=None):
    # FIXME(sirp): since this returns fixed_ips, this would be better named
    # fixed_ip_get_all_by_network.
    # NOTE(vish): The ugly joins here are to solve a performance issue and
    #             should be removed once we can add and remove leases
...
def network_in_use_on_host(context, network_id, host):
    fixed_ips = network_get_allocated_fixed_ips(context, network_id, host)
...
def block_device_mapping_get_all_by_instance(context, instance_uuid,
    ...
    return _block_device_mapping_get_query(context, use_slave=use_slave).\
                 filter_by(instance_uuid=instance_uuid).order_by(asc("id")).\
                 all()
...
@require_context
def huawei_instance_extra_get_by_instance_uuid(context, instance_uuid):
    return (model_query(context, models.HuaweiInstanceExtra).
            filter_by(instance_uuid=instance_uuid).
            first())


@require_context
def injected_files_get_by_instance_uuid(context, instance_uuid):
    result = model_query(context,
                         models.HuaweiInstanceExtra.injected_files,
                         base_model=models.HuaweiInstanceExtra). \
        filter_by(instance_uuid=instance_uuid).first()
    if not result:
        raise exception.InstanceNotFound(instance_id=instance_uuid)
    return result.injected_files


@require_context
def huawei_instance_extra_get_by_host(context, host):
    query = model_query(context, models.HuaweiInstanceExtra).join(
        (models.Instance, models.HuaweiInstanceExtra.instance_uuid
                          == models.Instance.uuid)).filter(
        models.Instance.deleted == 0).filter(
        models.Instance.host == host)
    return query.all()


@require_context
def huawei_instance_extra_create(context, instance_uuid, updates):
    session = get_session()
    with session.begin():
        extra = (model_query(context, models.HuaweiInstanceExtra).
                 filter_by(instance_uuid=instance_uuid).first())
        if not extra:
            extra = models.HuaweiInstanceExtra()
...
@require_admin_context     
def livemigrations_create(context, values):
    live_migrate_ref = models.HuaweiLiveMigrate()
    live_migrate_ref.update(values)
    live_migrate_ref.save()
    return live_migrate_ref


@require_admin_context 
def livemigrations_get_all(context, session=None):
    result = model_query(context, models.HuaweiLiveMigrate, session=session).all()
    return result


@require_context
def livemigrations_get_all_by_host(context, host, session=None):
    result = model_query(context, models.HuaweiLiveMigrate, session=session).\
        filter_by(dest_host=host).all()
    return result


@require_admin_context
def livemigrations_get_by_uuid(context, uuid, session=None):
    return model_query(context,
                       models.HuaweiLiveMigrate,
                       session=session).\
...
@require_admin_context 
def livemigrations_destroy(context, instance_id):
    model_query(context, models.HuaweiLiveMigrate).\
