...
class HuaweiInstanceExtra(BASE, NovaBase):
    __tablename__ = 'huawei_instance_extra'
    __table_args__ = (
        Index('instance_extra_idx', 'instance_uuid'),)
    id = Column(Integer, primary_key=True, autoincrement=True)
    instance_uuid = Column(String(36), ForeignKey('instances.uuid'),
                           nullable=False)
    vcpu_topology = Column(Text)
    scheduler_hints = Column(Text)
    core_bind = Column(Text)
    request_network = Column(Text)
    instance = orm.relationship(Instance,
                                backref=orm.backref('huawei_extra',
                                                    uselist=False),
                                foreign_keys=instance_uuid,
                                primaryjoin=instance_uuid == Instance.uuid)

    # json-encode string containing instance statistics
    stats = Column(Text, default='{}')
    injected_files = orm.deferred(Column(Text))

class HuaweiLiveMigrate(BASE, NovaBase):
    """Represents a live-migrate task."""
    __tablename__ = 'huawei_live_migrations'
    id = Column(Integer, primary_key=True)
    instance_uuid = Column(String(36))
    source_host = Column(String(255))
    dest_host = Column(String(255))
    dest_addr = Column(String(32))
    block_migration = Column(Boolean, default=False)
    migrate_data = Column(Text)
