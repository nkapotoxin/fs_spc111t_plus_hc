...
service_opts = [
    ...
    cfg.StrOpt('proxy_manager',
               default='nova.proxy.manager_proxy.ComputeManager',
               help='nova proxy Manager for compute'),
...
compute_proxy_opts = [
    cfg.IntOpt('workers',
               default= 1,
               help='The workers number of nova proxy'),
    ]
...
class Service(service.Service):
    ...
    def start(self):
        if self._is_multi_worker():
            self._start_multi_worker()
            return
        ...
        LOG.debug("Start RPC server for service %s", self.topic)
        self.tg.add_dynamic_timer(self.manager.update_rpcserver,
                context=ctxt,
                service=self)
    ...
    def stop(self):
        for host in self.rpcservers:
            rpcserver = self.rpcservers.get(host)
            if rpcserver:
                try:
                    rpcserver.stop()
                    rpcserver.wait()
    ...
    def update_rpcserver(self, host_list):
        """
        ...
        for host in self.rpcservers:
            if host not in host_list:
                to_remove_hosts.append(host)
        for host in to_remove_hosts:
            self.remove_host_rpc(host)
        
        for host in host_list:
            if host not in self.rpcservers:
                self.add_host_rpc(host)
                
        self.print_rpcserver()
    ...
    def remove_host_rpc(self, host):
        """Remove host rpc server for spec host."""
        LOG.info("Remove RPC server for host '%s'", host)
        if not self.rpcservers.has_key(host):
            LOG.warn("RPC server '%s' may has been removed." % host)
            return
        
        # Remove rpcserver
        rpcserver = self.rpcservers.pop(host)
        LOG.info("Remove RPC server for host '%s' end.", host)
    
    def print_rpcserver(self):
        LOG.info("Nova compute rpc servers list")
        for host in self.rpcservers:
            LOG.info("--- %s" % host)

    def _is_multi_worker(self):
        """
        Only used in multi-worker mode
        :return:
        """
        
        def _synchronized():
            lock_path = '/var/run/nova/lock/%s' % self.host
            utils.synchronized = functools.partial(
    ...
    def _is_main_worker(self):
        global fp
        fp = open('/etc/nova/nova-proxy/%s.lock' % self.host, 'w')
        try:
    ...
    def _start_multi_worker(self):
        ...
        self.tg.add_dynamic_timer(self.manager._heal_all_instances_state,
                                  context=context.get_admin_context())
        # Add service to the ServiceGroup membership group.
        LOG.debug("Join ServiceGroup membership for this service %s",
            self.topic)
