...
from nova.huawei.objects import volume_connection as volume_connection_obj
cinder_opts = [
...
class API(object):
        ...
        if volume['shareable'] == 'true':
            if (volume['status'] != "available"
               and volume['status'] != 'in-use'):
                msg = _("shareable volume's status must be"
                        "'available' or 'in-use'")
                raise exception.InvalidVolume(reason=msg)
            if instance:
                for attachment in volume['attachments']:
                    if attachment['instance_uuid'] == instance['uuid']:
                        msg = _("the shareable volume has already been attached"
    ...
    @utils.synchronized("check_volume_connection")
    @translate_volume_exception
    def initialize_connection(self, context, volume_id, connector,
                              instance_uuid, host):

        volume_connection_obj.VolumeConnection.set(context,
                                                   volume_id,
                                                   instance_uuid,
                                                   host)
        return cinderclient(context,http_timeout=300).volumes.initialize_connection(volume_id,
                                                                   connector)
    ...
    def terminate_connection(self, context, volume_id, connector,
        ...
        try:
            ...
            if connection_info is None:
                LOG.info("Vol %(vol_id)s and vm%(uuid)s without conn_info" %
                         {"vol_id": volume_id,
                          "uuid": instance_uuid})
                return
        except exception.VolumeBDMNotFound:
            LOG.info("Bdm of vol %(vol_id)s and vm%(uuid)s disappeared" %
                     {"vol_id": volume_id,
                      "uuid": instance_uuid})
            return
        volume_connection_obj.VolumeConnection.unset(context,
                                                     volume_id,
                                                     instance_uuid,
                                                     host)
        connection_num = volume_connection_obj.VolumeConnection.count(
            context, volume_id, host)
        if connection_num == 0:
            LOG.info("Zero Connection of volume %(volume_id)s on %(host)s,"
                     " send msg to cinder" % {"volume_id": volume_id,
                                              "host": host})
            return cinderclient(context,http_timeout=300).volumes.terminate_connection(
                volume_id, connector)
        else:
            LOG.info("%(connection_num)d Connections of volume %(volume_id)s "
                     "on %(host)s left, do not send msg to cinder" %
    ...
    @translate_volume_exception    
    def upload_to_image(self, context, volume_id, force , image_name, container_format,
                        disk_format):
        return cinderclient(context).volumes.upload_to_image(volume_id, force, image_name, container_format,
                        disk_format)
