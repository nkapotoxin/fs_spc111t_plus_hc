        ...
        for fixedip in fixedips:
            if fixedip.allocated:
                ...
                if fixedip.default_route:
                    if not CONF.dnsmasq_config_file:
...
def restart_dhcp(context, dev, network_ref, fixedips):
    ...
    if pid:
        ...
        # of the file itself
        if conffile.split('/')[-1] in out:
            out = out[out.index('dnsmasq'):].\
                replace(binascii.a2b_hex('00'), '')
            if out == "".join(itertools.chain(cmd)):
                try:
                ...
                try:
                    ...
                    LOG.error(_('Kill dnsmasq threw %s'), exc)
        else:
