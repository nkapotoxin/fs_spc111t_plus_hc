...
class DbDriver(api.ServiceGroupDriver):
    ...
    def _report_state(self, service):
        ...
        ctxt = context.get_admin_context()
        state_catalog = {}
        service_ref_tmp = None
        try:
            report_count = service.service_ref['report_count'] + 1
            state_catalog['report_count'] = report_count

            # note(HUAWEI) it maybe return None
            service_ref_tmp = self.conductor_api.service_update(ctxt,
                                   service.service_ref, state_catalog)
            if service_ref_tmp:
                service.service_ref = service_ref_tmp
            else:
                LOG.error(_("the result get from the db is is None"))
                raise
