...
def del_scsi_device(host_id, lun_id):
    ...
    for device in device_list:
        ...
        if os.path.exists('/proc/nxup_lun_map_a') and num_list[0] == host_id and num_list[3] == lun_id:
            ...
            out = None
            try:
                out, err = utils.execute('grep', pattern_str, '/proc/nxup_lun_map_a', run_as_root=True)
            except Exception as e:  #not found, delete it
                LOG.info("delete device %s" % device)
                echo_scsi_command("/sys/class/scsi_device/%s/device/delete" % device, "1")

def rescan_hosts(hbas,lun_id=None):
    for hba in hbas:
        try:
            if lun_id is None or not str(lun_id).isdigit():
                ...
                del_scsi_device(host_id, str(lun_id))
                echo_scsi_command("/sys/class/scsi_host/%s/scan"
...
def get_scsi_wwn(dev_path):
        """Get the LUN WWN for Fibre Channel device"""
        real_path = os.path.realpath(dev_path)

        out = None
        try:
            out, err = utils.execute('scsi_id', '--whitelisted', '-d', real_path,
                                     run_as_root=True)
