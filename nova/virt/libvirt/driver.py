...
class LibvirtDriver(driver.ComputeDriver):
    ...
    @hooks.add_hook("compute_driver_init_host_hook")
    ...
    @hooks.add_hook("undefine_domain_hook")
    ...
    def attach_volume(self, context, connection_info, instance, mountpoint,
        ...
        try:
            ...
            # add address and target number for virtio-scsi disk
            from nova.huawei.virt.libvirt import utils as hw_utils
            disk_xml = hw_utils.add_address_for_virtio_scsi_disk(
                                            virt_dom.XMLDesc(0), conf.to_xml())
    ...
    @hooks.add_hook("soft_reboot_hook")
    ...
    def _create_image(self, context, instance,
        ...
        libvirt_utils.write_to_file(
            self._get_console_log_path(instance), '', 31)
        ...
        if configdrive.required_by(instance):
            ...
            with configdrive.ConfigDriveBuilder(instance_md=inst_md) as cdb:
                ...
                if not (os.path.exists(configdrive_path) and
    ...
    def _get_guest_cpu_config(self, flavor, image, guest_cpu_numa):
        ...
        if guest_cpu_numa:
            for instance_cell in guest_cpu_numa.cells:
                instance_cell.id=guest_cpu_numa.cells.index(instance_cell)
    ...
    def _get_guest_config(self, instance, network_info, image_meta,
        ...
        #this for loop moved and modified by huawei for network order
        for vif in network_info:
            config = self.vif_driver.get_config(
                instance, vif, image_meta,
                flavor, CONF.libvirt.virt_type)
            if config is not None:
                if vif["meta"].get("pci_slotnum"):
                    net_start_pci = vif["meta"]["pci_slotnum"]
                    config.set_pci_slot(net_start_pci)
        ...
        if ((CONF.vnc_enabled and
            ...
            # remove this to guarantee vnc login
