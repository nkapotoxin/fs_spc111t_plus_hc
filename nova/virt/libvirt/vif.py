...
class LibvirtGenericVIFDriver(object):
    ...
    def get_base_config(self, instance, vif, image_meta,
                        inst_type, virt_type):
        from nova.huawei.virt.libvirt import config as hw_vconfig
        conf = hw_vconfig.LibvirtConfigGuestInterface()
    ...
    def get_trunk_br_name(self, iface_id):
        return ("tbr" + iface_id)[:network_model.NIC_NAME_LEN]

    def get_patch_pair_names(self, iface_id):
        return (("pvm%s" % iface_id)[:network_model.NIC_NAME_LEN],
                ("pin%s" % iface_id)[:network_model.NIC_NAME_LEN])
    ...
    def get_ebr_veth_pair_names(self, iface_id):
        return (("qvp%s" % iface_id)[:network_model.NIC_NAME_LEN],
                ("qve%s" % iface_id)[:network_model.NIC_NAME_LEN])

    def get_vm_pcy_ebr_name(self, iface_id):
        return ("qep%s" % iface_id)[:network_model.NIC_NAME_LEN]

    def get_vm_pcy_tap_name(self, iface_id):
    ...
    def get_config_ovs_trunk(self, instance, vif, image_meta,
                              inst_type, virt_type):
        conf = self.get_base_config(instance, vif, image_meta,
                                    inst_type, virt_type)

        designer.set_vif_host_backend_ovs_config(
            conf, self.get_trunk_br_name(vif['id']),
            self.get_ovs_interfaceid(vif),
            self.get_vif_devname(vif))
    ...
    def get_config_ovs(self, instance, vif, image_meta,
                       inst_type, virt_type):
        if vif.is_trunk_plug_enable():
            return self.get_config_ovs_trunk(instance, vif,
                                              image_meta,
                                              inst_type,
                                              virt_type)
        elif self.get_firewall_required(vif) or vif.is_hybrid_plug_enabled():
    ...
    def plug_ovs_trunk(self, instance, vif):
        """Plug using trunk strategy

        Create a per-VIF OVS bridge, then link that bridge to the OVS
        integration bridge via an patch port device.
        """
        try:
            ...
            linux_net.create_ovs_bridge(br_name)

            linux_net.create_ovs_patch_port(br_name, patch_port1, patch_port2)
            linux_net.create_ovs_patch_port(bridge_name, patch_port2, patch_port1)
        except processutils.ProcessExecutionError:
            LOG.error(_LE("Failed while plugging vif"),
                          instance=instance)


    def plug_ovs(self, instance, vif):
        if vif.is_trunk_plug_enable():
            self.plug_ovs_trunk(instance, vif)
        elif self.get_firewall_required(vif) or vif.is_hybrid_plug_enabled():
    ...
    def unplug_ovs(self, instance, vif):
        if vif.is_trunk_plug_enable():
            self.unplug_ovs_trunk(instance, vif)
        elif self.get_firewall_required(vif) or vif.is_hybrid_plug_enabled():
    ...
    def unplug_hw_veb(self, instance, vif):
        try:
            ...
            profile = vif["profile"]
            pci_vendor_id = profile['pci_vendor_info'].split(':')[0]
            pf_eth = pci_utils.get_pf_by_vfpci(profile['pci_slot'])
            vf_index = pci_utils.get_vf_index(profile['pci_slot'])
            if pci_vendor_id in pci_devspec.SUPPORTED_VENDORS:
                status, output = commands.getstatusoutput('ip link show %s | grep "vf %s"' %(pf_eth, vf_index))
                if not status:
                    if "65535" in output:
                        eth_index = int(pf_eth.split('h')[1]) + 1
                        pf_eth = "eth" + str(eth_index)
            LOG.debug("unplug_hw_veb:mac:%s,vlan:%s,profile['pci_slot']:%s,pf_eth:%s,vf_index:%s"%(mac,vlan,profile['pci_slot'],pf_eth,vf_index))
            ...
            LOG.exception(_LE("Failed while unplug_hw_veb vif"),
                          instance=instance)
