...
class FilterScheduler(driver.Scheduler):
    ...
    @hooks.add_hook("select_destinations_hook")
    ...
    def _check_fulfill_for_multiple_create(self, context, num_instances,
                                           selected_hosts):
        # Couldn't fulfill the request_spec
        if len(selected_hosts) < num_instances:
            # NOTE(Rui Chen): If multiple creates failed, force to update
            # HostStates in order to release the resource consumed by
            # instance in the process of selecting host. See details:
            # https://bugs.launchpad.net/nova/+bug/1408859
            if num_instances > 1:
