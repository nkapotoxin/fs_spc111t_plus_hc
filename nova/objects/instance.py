...
db_query_opts = [
    cfg.IntOpt("instance_numbers_per_paging_query",
        default=1000,
        help="Number of instances when query db in paging way")
]
CONF.register_opts(db_query_opts)
...
def _paging_query_with_filters(context, filters, expected_attrs=None,
                               use_slave=False):
    db_inst_list = []

    marker = None
    limit = CONF.instance_numbers_per_paging_query
    while True:
        paging_db_inst_list = objects.InstanceList.get_by_filters(
            context, filters=filters, limit=limit, marker=marker,
            expected_attrs=expected_attrs, use_slave=use_slave)
        db_inst_list += paging_db_inst_list
        if len(paging_db_inst_list) < limit:
            break
        marker = paging_db_inst_list[-1]['uuid']
    return db_inst_list
...
class InstanceList(base.ObjectListBase, base.NovaObject):
    ...
    def get_by_host(cls, context, host, expected_attrs=None, use_slave=False):
        filters={'host': host, 'deleted': False}
        db_inst_list = _paging_query_with_filters(context, filters,
            expected_attrs=expected_attrs, use_slave=use_slave)
        return _make_instance_list(context, cls(), db_inst_list,
            expected_attrs)
    ...
    def get_by_host_and_node(cls, context, host, node, expected_attrs=None):
        filters={'host': host, 'node': node, 'deleted': False}
        db_inst_list = _paging_query_with_filters(context, filters,
            expected_attrs=expected_attrs)
        return _make_instance_list(context, cls(), db_inst_list,
                                   expected_attrs)
