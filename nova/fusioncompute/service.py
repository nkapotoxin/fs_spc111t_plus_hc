...
class Service(service.Service):
    ...
    def start(self):
        if self._is_fc_nova_compute():
            return self._start_compute_worker()
    ...
    def _is_fc_nova_compute(self):
        """
        Only used for fc-nova-compute in multi-worker mode
        :return:
        """
        ...
        try:
            if 'FusionComputeDriver' in CONF.compute_driver \
            and CONF.fusioncompute.workers > 1:
                _synchronized()
                return True
        except Exception:
            return False
    ...
    def _is_main_worker(self):
        global fp
        fp = open('/etc/nova/fc-nova-compute/%s.lock' % self.host, 'w')
        try:
