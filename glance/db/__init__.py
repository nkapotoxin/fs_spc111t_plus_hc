...
class ImageRepo(object):
    ...
    def save(self, image, from_state=None):
        image_values = self._format_image_to_db(image)
        props_to_delete = image.extra_properties.deleted
        props_to_update = image.extra_properties.modified
        attrs_to_update = image.get_modified()
        if image_values['size'] > CONF.image_size_cap:
            raise exception.ImageSizeLimitExceeded
        try:
            from_state = from_state if from_state else \
                image.get_initial_value('status')
            new_values = self.db_api. \
                image_update(self.context,
                             image.image_id,
                             image_values,
                             purge_props=False,
            ...
            msg = _("NoFound image found with ID %s") % image.image_id
            raise exception.NotFound(msg)
        except (exception.Forbidden):
            msg = _("Forbidden image found with ID %s") % image.image_id
            raise exception.NotFound(msg)
        except (exception.Conflict):
            msg = _("Conflict image found with ID %s") % image.image_id
            raise exception.NotFound(msg)
    ...
    def remove(self, image):
        ...
        try:
            self.db_api.image_update(self.context,
                                     ...
                                     props_to_update=props_to_update,
                                     props_to_delete=props_to_delete)
        except (exception.NotFound, exception.Forbidden, exception.Conflict):
