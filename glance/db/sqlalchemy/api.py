...
def image_get_all(context, filters=None, marker=None, limit=None,
    ...
    # added admin_as_user filter for v2 api, huawei feature
...
def _image_update(context, values, image_id, purge_props=False,
    ...
    # NOTE(ativelkov): attrs_to_update may contain 'visibility' attribute.
    # For backwards compatibility it is mapped to 'is_public' DB attribute.
    # So, if 'visibility' is present in the set of modified attributes, it
    # should be replaced with 'is_public'
    if attrs_to_update and 'visibility' in attrs_to_update:
        attrs_to_update = set(attrs_to_update)
        attrs_to_update.remove('visibility')
        attrs_to_update.add('is_public')
        ...
        if image_id:
            ...
            for k in keys:
                if k not in image_ref.to_dict():
                    del values[k]
                # NOTE (ativelkov): if attrs_to_update set is passed remove
                # the unmodified attributes from the update query
                if attrs_to_update and k not in attrs_to_update:
                    del values[k]
...
def _set_properties_for_image(context, image_ref, properties,
                              purge_props=False, props_to_update=None,
                              props_to_delete=None,
                              session=None):
    """
    ...
    for name, value in six.iteritems(properties):
        # NOTE(ativelkov): if props_to_update set is passed, update/create only
        # the modified properties
        if props_to_update is not None and name not in props_to_update:
            continue
    ...
    elif props_to_delete:
        for key in props_to_delete:
            if key in orig_properties:
                prop_ref = orig_properties[key]
                image_property_delete(context, prop_ref.name, image_ref.id,
                                      session=session)
