...
class StateTrackingObject(object):
    _modified_properties = collections.defaultdict(list)

    def __setattr__(self, key, value):
        if not key.startswith('_') and hasattr(self, key):
            self._modified_properties[key].append(getattr(self, key))
        super(StateTrackingObject, self).__setattr__(key, value)

    def reset(self):
        self._modified_properties.clear()

    def get_modified(self):
        return set(self._modified_properties.keys())

    def get_modified_values(self):
        return self._modified_properties

    def get_initial_value(self, key):
        if self._modified_properties[key]:
            return self._modified_properties[key][0]
        return getattr(self, key)


class Image(StateTrackingObject):
    ...
    def reset(self):
        self.extra_properties.reset()
        super(Image, self).reset()
