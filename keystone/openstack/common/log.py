...
class FSSysLogHandler(logging.handlers.SysLogHandler):
    def __init__(self, facility):
        #super(FSSysLogHandler, self).__init__(address="localhost",facility=facility)
    ...
    def emit(self, record):
        ...
        if logging.handlers._unicode and type(msg) is unicode:
            ...
            if logging.handlers.codecs:
                ...
                msg = msg
        try:
            #handle token encrypt.
            if "middleware.py" in msg:
                if ("Matched GET /tokens/" in msg and not msg.endswith("revoked")) or\
                        ('\'path_info\': \'/tokens/' in msg and not msg.endswith('revoked\'}')):
                    Pos = msg.find("/tokens/")+8
                    logToken = msg[Pos:Pos+32]
                    encodedToken = b64encodeToken(logToken)
                    msg = msg.replace(logToken,encodedToken)
                if "Match dict" in msg and "token_id" in msg:
                    Pos = msg.find("token_id")+13
                    logToken = msg[Pos:Pos+32]
                    encodedToken = b64encodeToken(logToken)
                    msg = msg.replace(logToken,encodedToken)
            elif "log.py" in msg:
                if '{\'param_1\': \'' in msg or '{\'param_1\': u\'' in msg:
                    Pos = msg.find('{\'param_1\': \'')+13
                    if Pos-13==-1:
                        Pos = msg.find('{\'param_1\': u\'')+14
                    logToken = msg[Pos:Pos+32]
                    encodedToken = b64encodeToken(logToken)
                    msg = msg.replace(logToken,encodedToken)
                if "access" in msg and "issued_at" in msg:
                    startPos = msg.find('"id": ',msg.find('access'))+7
                    endPos = msg.find('",',startPos)
                    logToken = msg[startPos:endPos]
                    encodedToken = b64encodeToken(logToken)
                    msg = msg.replace(logToken,encodedToken)
            if hasSensitiveStr(msg):
                return
