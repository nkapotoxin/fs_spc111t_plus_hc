...
class TokenModel(sql.ModelBase, sql.DictBase):
    ...
    id = sql.Column(sql.String(640), primary_key=True)
...
class Token(token.persistence.Driver):
    ...
    def get_token(self, token_id):
        ...
        tid = crypt.token_encrypt(token_id)
        token_ref = session.query(TokenModel).get(tid)
        if not token_ref or not token_ref.valid:
            raise exception.TokenNotFound(token_id=token_id)
        token_ref = token_ref.to_dict()
        token_ref['id'] = token_id
        try:
            if 'access' in token_ref['token_data']:
                token_ref['token_data']['access']['token']['id'] = crypt.decrypt(token_ref['token_data']['access']['token']['id'])
            token_ref['key'] = crypt.decrypt(token_ref['key'])
        except KeyError:
            pass
        return token_ref
    ...
    def create_token(self, token_id, data):
        ...
        data_copy['id'] = crypt.token_encrypt(data_copy['id'])
        try:
            # v2.0 token
            if 'access' in data_copy['token_data']:
                data_copy['token_data']['access']['token']['id'] = crypt.token_encrypt(data_copy['token_data']['access']['token']['id'])

            if CONF.token.id_no_catalog:
                LOG.info("id_no_catalog:%s" % CONF.token.id_no_catalog)
                # v2.0 token
                if 'access' in data_copy['token_data']:
                    # process problem that token is too long
                    data_copy['token_data']['access'].pop('serviceCatalog', None)
                # v3 token
                if 'token' in data_copy['token_data']:
                    # process problem that token is too long
                    data_copy['token_data']['token'].pop('catalog', None)
            data_copy['key'] = crypt.token_encrypt(data_copy['key'])
        except KeyError:
            pass
        ...
        token_ref = token_ref.to_dict()
        token_ref['id'] = crypt.decrypt(token_ref['id'])

        try:
            operationlog.info("User: %s, created token: %s, at time: %s"%(token_ref['user_id'],
                                                                          HWExtend.b64encodeToken(token_ref['id']),
                                                                          timeutils.utcnow()), 
                                                                          extra={"type":"operate"})
        except:
            try:
                operationlog.info("User: %s, created token at time: %s"%(token_ref['user_id'],
                                                                          timeutils.utcnow()), 
                                                                          extra={"type":"operate"})
            except Exception as e:
                operationlog.info("Something went wrong, user can't be logged. %s."%e,extra={"type":"operate"})
            try:
                operationlog.info("created token: %s, at time: %s"%(HWExtend.b64encodeToken(token_ref['id']),
                                                                    timeutils.utcnow()), 
                                                                    extra={"type":"operate"})
            except Exception as e:
                operationlog.info("Something went wrong, token can't be logged. %s"%e,extra={"type":"operate"})
        try:
            if 'access' in token_ref['token_data']:
                token_ref['token_data']['access']['token']['id'] = crypt.decrypt(token_ref['token_data']['access']['token']['id'])
            token_ref['key'] = crypt.decrypt(token_ref['key'])
    ...
    def _list_tokens(self, user_id, tenant_id=None, trust_id=None,
        ...
        if not CONF.token.revoke_by_id:
            return []
        if trust_id:
            tokens = self._list_tokens_for_trust(trust_id)
        if consumer_id:
            tokens = self._list_tokens_for_consumer(user_id, consumer_id)
        else:
            tokens = self._list_tokens_for_user(user_id, tenant_id)
        for i in range(len(tokens)):
            tokens[i] = crypt.decrypt(tokens[i])
        return tokens
