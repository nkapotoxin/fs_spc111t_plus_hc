...
class ISCSIConnector(InitiatorConnector):
    ...
    def __init__(self, root_helper, driver=None,
        ...
        self.use_ultrapath = kwargs.get('use_ultrapath', False)
    ...
    def connect_volume(self, connection_properties):
        ...
        """
        if self.use_ultrapath and connection_properties.get('description','') == 'huawei':
            return self.connect_volume_ultrapath(connection_properties)
        ...
        base_device = ("/dev/disk/by-path/ip-%s-iscsi-%s-lun-%s" %
        ...
        while not os.path.exists(host_device):
            # if the lunid is used, we need to remove invalid device.
            if os.path.exists(base_device):
                LOG.warn("%s not exist, but %s exist." % (host_device, base_device))
                dev_name = self._linuxscsi.get_name_from_path(base_device)
    ...
    def _get_hw_multipath_device_name(self, detail, bypath=False):
        if not bypath:
            return "/dev/disk/by-id/wwn-0x%s" % detail['lun_wwn']
        # if host connects mutiple ip-san devices, this MUST modefy.
        files = []
    ...
    def _smartscan_ultrapath(self,iscsi_properties, del_params=""):
        ...
        (out, err) = self._execute('sh', '/etc/FSComponentUtil/commShell/smartscan.sh',
    ...
    def _smartdelete_ultrapath(self,iscsi_properties):
        ...
        (out, err) = self._execute('sh', '/etc/FSComponentUtil/commShell/private_smartdelete.sh',
                                       str(iscsi_properties['target_lun']),
                                       ','.join(iscsi_ip_list),
                                       run_as_root=True,
                                       root_helper=self._root_helper,
    ...
    def _ultrapath_connect_to_iscsi_portal(self, connection_properties,exception_list=[],login_info = []):
        ...
        for index in range(int(connection_properties['target_num'])):
            ...
            if len(portals) == 0 or len([s for s in portals
                ...
                try:
                    ...
                    if exc.exit_code in [21, 255]:
                        try:
                            self._run_iscsiadm(props, ('--op', 'new'))
    ...
    def connect_volume_ultrapath(self, connection_properties):
        """Attach the volume to instance_name.
        ...
        def clean_device(dev_path, properties):
            ...
            while os.path.exists(dev_path):
                dev_name = self._linuxscsi.get_name_from_path(dev_path)
                self._remove_scsi_device(dev_name)
                del_tries += 1
                if del_tries > 10:
                    self._execute('rm',
                                  '-f',
        ...
        self._ultrapath_connect_to_iscsi_portal(connection_properties, exception_list,login_info)

        if len(exception_list) == int(connection_properties['target_num']):
            LOG.error("All paths don't work normally.")
            raise

        #self._rescan_iscsi()
        host_device = self._get_hw_multipath_device_name(connection_properties)

        # The /dev/disk/by-path/... node is not always present immediately
        # TODO(justinsb): This retry-with-delay is a pattern, move to utils?
        login_flag = True if len(login_info) == int(connection_properties['target_num']) else False
        tries = 0
        del_params = "SCAN"
        first_clean = True
        while not os.path.exists(host_device) or not _check_valid_device(host_device):
            '''
            base_devices = self._get_hw_multipath_device_name(connection_properties, True)
            ...
            for bydev in base_devices:
                ...
                dev_name = self._linuxscsi.get_name_from_path(bydev)
                self._remove_scsi_device(dev_name)
            '''
            if first_clean and not login_flag:
                clean_device(host_device, connection_properties)
                first_clean = False
            ...
            if tries >= self.device_scan_attempts:
                raise exception.VolumeDeviceNotFound(device=host_device)
            if tries > 0:
                LOG.warn(_("ISCSI ultrapath volume not yet found at: %(host_device)s. "
                           "Will rescan & retry.  Try number: %(tries)s"),
                         {'host_device': host_device,
                          'tries': tries})
            '''
            # The rescan isn't documented as being necessary(?), but it helps
            for index in range(int(connection_properties['target_num'])):
                if connection_properties['target_iqn'][index] in valid_list:
                    cp = {'target_iqn':connection_properties['target_iqn'][index],
                          'target_portal':connection_properties['target_portal'][index]}
                    self._run_iscsiadm(cp,  ("--rescan",))
                #self._resan_ultrapath()
            '''
            if not login_flag:
                self._smartscan_ultrapath(connection_properties, del_params)
            tries = tries + 1
            if not os.path.exists(host_device):
                time.sleep(tries ** 2)

        if tries != 0:
            LOG.debug(_("Found iSCSI ultrapath node %(host_device)s "
                        "(after %(tries)s rescans)"),
                      {'host_device': host_device, 'tries': tries})
    ...
    def disconnect_volume_ultrapath(self, connection_properties, device_info):
        #self._rescan_iscsi()
        host_device = self._get_hw_multipath_device_name(connection_properties)
        ...
        if dev_name:
            ...
            self._remove_scsi_device(dev_name)
            while True:
                if os.path.exists(device_info["path"]):
                    self._remove_scsi_device(dev_name)
                    flag = flag + 1
                else:
                    break
                if flag == 10:
                    LOG.error("ERROR!The path still exist. path=%s, realpath=%s"
                              % (device_info["path"], dev_name))
                    LOG.error("force to delete the path.")
                    os.unlink(device_info["path"])
                    break
                time.sleep(1)

        # TODO:not very good
        device_prefix = "/dev/disk/by-path/scsi-"
    ...
    def disconnect_volume(self, connection_properties, device_info):
        ...
        if self.use_ultrapath and connection_properties.get('description','') == 'huawei':
            return self.disconnect_volume_ultrapath(connection_properties, device_info)
        ...
        if dev_name:
            ...
            while True:
                if os.path.exists(device_info["path"]):
                    rpath = os.path.realpath(device_info["path"])
                    self._linuxscsi.remove_scsi_device(rpath)
                    flag = flag + 1
                else:
                    break
                if flag == 10:
                    LOG.error("ERROR!The path still exist. path=%s, realpath=%s"
                              % (device_info["path"], rpath))
                    LOG.error("force to delete the path.")
                    os.unlink(device_info["path"])
                    break
                time.sleep(1)
    ...
    def _remove_scsi_device(self, dev_name):
        ...
        (out, err) = self._execute('sh', '/etc/FSComponentUtil/commShell/smartdelete.sh',
                                   device_name,
                                   run_as_root=True,
                                   root_helper=self._root_helper,
                                   check_exit_code=0)
    def _get_device_path(self, connection_properties):
        if connection_properties.get('description') == 'huawei':
            return self._get_hw_multipath_device_name(connection_properties)
        else:
    ...
    def _connect_to_iscsi_portal(self, connection_properties, login_info=[]):
    ...
    def _clean_ultrapath(self):
        try:
            self._execute('hot_add', '-f',
                           run_as_root=True,
                           root_helper=self._root_helper,
                           check_exit_code=0)
        except Exception as e:
            LOG.error("hot_add -f error.%s" % e)

    def _resan_ultrapath(self):
        try:
            self._execute(('upRescan'),
                           run_as_root=True,
                           root_helper=self._root_helper,
                           check_exit_code=0)
        except Exception as e:
            LOG.error("upRescan error. %s" % e)

    def _test_ipadrress(self, ip):
        try:
            self._execute('ping',ip,'-i','0.2','-c','3', '-w', '1',
                          run_as_root=True,
                          root_helper=self._root_helper,
                          check_exit_code=0)
            return True
...
class FibreChannelConnector(InitiatorConnector):
    ...
    def _get_current_ultrapath_devs(self, detail):
        # if host connects mutiple ip-san devices, this MUST modefy.
        files = []
        dir = "/dev/disk/by-path/"
        if os.path.isdir(dir):
    ...
    def connect_volume(self, connection_properties):
        ...
        def _check_valid_device(path):
            ...
            out, info = None, None
            try:
                out, info = self._execute(*cmd, run_as_root=True, root_helper=self._root_helper, check_exit_code=0)
            except putils.ProcessExecutionError as e:
                LOG.error(_("Failed to access the device on the path "
                            "%(path)s: %(error)s.") %
                          {"path": path, "error": e.stderr})
        ...
        def _wait_for_device_discovery(host_devices):
            tries = self.tries
            if self.use_ultrapath:
                if self.tries > 0:
                    time.sleep(self.tries ** 2)
                ultra_dev_path = "/dev/disk/by-id/wwn-0x%s" % connection_properties['lun_wwn']
                if os.path.exists(ultra_dev_path):
                    ...
                    #check is it valid
                    by_path_devs = self._get_current_ultrapath_devs(connection_properties)
                    if len(by_path_devs) > 1:
                        LOG.error("found multiple by-path link."
                                  "Host may connect multiple array.")
                    for del_dev in by_path_devs:
                        if connection_properties['lun_wwn'] == self._linuxfc.get_scsi_wwn(del_dev):
            ...
            self._linuxfc.rescan_hosts(hbas,lun_id=connection_properties.get('target_lun', None))
    ...
    def disconnect_volume(self, connection_properties, device_info):
        ...
        """
        if self.use_ultrapath:
            host_device = "/dev/disk/by-id/wwn-0x%s" % connection_properties['lun_wwn']
