...
class TseriesCommon():
    ...
    def _get_login_info(self):
        ...
        for key in ['UserName', 'UserPassword']:
            ...
            if node_text.find('!$$$') > -1:
                ...
                if key == "UserPassword":
                    logininfo_key = crypt.decrypt(logininfo_key)
                logininfo[key] = logininfo_key
            else:
                if key == "UserPassword":
                    logininfo[key] = crypt.decrypt(node_text)
                else:
    ...
    def _get_maximum_capacity_pool_id(self, pools_conf, pools_dev, luntype):
        ...
        if luntype == 'Thick':
            detailed_pool = {}
            detailed_pools = []
            key = 'MaximumCapacity(MB)'
            for pool_dev in pools_dev:
                detailed_pool = self.get_pool_details(luntype, pool_dev[0])
                detailed_pools.append(detailed_pool)

            for pool in detailed_pools:
                pool[key] = int(pool[key])

            detailed_pools.sort(key=itemgetter(key))
            while len(detailed_pools) > 0:
                pool = detailed_pools.pop()
                if pool['Name'] in pools_conf:
                    return pool['ID']

        if luntype == 'Thin':
            nameindex, sizeindex = (1, 4)
        ...
        while True:
            try:
                ...
                while True:
                    ...
                    out = ssh_read(user, ssh_client.chan, cmd, 120)
                    if out.find('(y/n)') > -1 or out.find('y or n') > -1:
                        cmd = 'y'
                    elif out.find('The system is busy') > -1 and busyRetryTime > 0:
                        busyRetryTime = busyRetryTime - 1
                        LOG.info(_("System is busy, retry after sleep 10s."))
                        time.sleep(10)
                ...
                if connect_times < 2:
                    ...
                    #set ssh_pool as None when connect error,or the next command connect will also error. 
                    self.ssh_pool = None
        ...
        if not hostlun_id:
            ...
            while self._is_lun_normal(volume_id) is False:
                if count >= max_wait_time:
                    err_msg = (_('LUN %s is still not normal after'
                                 ' %(max_wait_time)s wait.')
                               % {'volume_id': volume_id,
                                  'max_wait_time': max_wait_time})
                    LOG.error(err_msg)
                    raise exception.VolumeBackendAPIException(data=err_msg)
                else:
                    LOG.debug(_('LUN %s is not normal, sleep 1s to wait.')
                              % volume_id)
                    time.sleep(1)
                    count = count + 1
            ...
            out = self._execute_cli(cli_cmd)
            # Check whether the hostlunid have already asigned.
            condition = re.search('host LUN is mapped or does not exist',
                                  out)
            while condition:
    ...
    def _get_all_host_map_info(self):
        """Get map infomation of all host."""
    ...
    def force_remove_map(self,volume):
        volume_id = volume.get('provider_location',None)
        if (volume_id is None) or not self._check_volume_created(volume_id):
            err_msg = (_('force_remove_volume: Volume %(name)s does not exist.')
                       % {'name': volume['name']})
            LOG.warn(err_msg)
            raise exception.VolumeNotFound(volume_id=volume_id if volume_id is not None else "unknown")
        
        map_id = None
        map_info = self._get_all_host_map_info()
        if map_info:
            for maping in map_info:
                if maping[2] == volume_id:
                    map_id = maping[0]
                    if map_id is not None:
                        self._delete_map(map_id,attempts=12)
                    else:
        ...
        if map_id is not None:
            try:
                ...
                if map_info and filter(lambda x:x[2] == volume_id,map_info):
                    err_msg = (_('remove_map: Failed to delete host map to volume'
                                 '%s.')
                               % volume_id)
                    LOG.error(err_msg)
                    raise exception.VolumeBackendAPIException(data=err_msg)
    ...
    def _delete_hostport(self, portid, attempts=5):
        ...
        while True:
            ...
            # IOs accessing the system.
            if (re.search('the initiator has requests to be performed', out) and
                    (attempts > 0)):

                LOG.debug(_('delhostport: There are IOs accessing '
    ...
    def _update_volume_stats(self):
        ...
        data['total_capacity_gb'] = self._get_total_capacity()
    ...
    def _get_total_capacity(self):
        ...
        for pool_dev in pools_dev:
            for pool_conf in pools_conf:
                if ((lun_type == 'Thick') and (pool_dev[5] == pool_conf)):
                    pool_details = self.get_pool_details(lun_type, pool_dev[0])
                    total_capacity += float(pool_details[key])
                    break
                elif ((lun_type == 'Thin') and (pool_dev[1] == pool_conf)):
                    pool_details = self.get_pool_details(lun_type, pool_dev[0])
                    total_capacity += float(pool_details[key])
                    break

        return total_capacity / 1024
