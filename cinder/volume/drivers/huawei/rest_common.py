...
class RestCommon():
    ...
    def _check_lun_exist(self, lun_id):
        ...
        if error_code == ERROR_CONNECT_TO_SERVER or error_code == ERROR_UNAUTHORIZED_TO_SERVER:
            LOG.error(_LE("cant't open the recent url, relogin."))
            self.login()
            result = self.call(url, data, "GET")

        error_code = result['error']['code']
        if error_code == ERROR_CONNECT_TO_SERVER or error_code == ERROR_UNAUTHORIZED_TO_SERVER:
            msg = _('\nCheck lun exist error.')
            LOG.error(msg)
            raise exception.CinderException(msg)
    ...
    def _mapping_hostgroup_and_lungroup(self, volume_name, hostgroup_id,
    ...
    def initialize_connection_iscsi(self, volume, connector):
        ...
        if connector.has_key('nova_use_ultrapath'):
            if connector.get('nova_use_ultrapath'):
                return self.initialize_ultrapath_connection_iscsi(volume, connector)
            else:
                return self.initialize_common_connection_iscsi(volume, connector)
    ...
    @utils.synchronized('huawei', external=True)
    def initialize_ultrapath_connection_iscsi(self, volume, connector):
        """Map a volume to a host and return target iSCSI information."""
        ...
        LOG.info(_LI('initialize_connection_iscsi, ips info: %s'), ips_info)

        # Create host_group if not exist.
        host_name = str(hash(connector['host']))
        ...
        hostlunid, lun_wwn = self._find_host_lun_id(hostid, lun_id)
    ...
    @utils.synchronized('huawei', external=True)
    def initialize_common_connection_iscsi(self, volume, connector):
        """Map a volume to a host and return target iSCSI information."""
        ...
        (iscsi_iqn, target_ip, portgroup_id) = self._get_iscsi_params(connector)
        LOG.info(_LI(
    ...
    def initialize_connection_fc(self, volume, connector):
        ...
        lun_id = self._mapping_hostgroup_and_lungroup(volume_name,
                                                      hostgroup_id, hostid)
        host_lun_id, lun_wwn = self._find_host_lun_id(hostid, lun_id)
        ...
        info = {'driver_volume_type': 'fibre_channel',
                'data': {'target_lun': int(host_lun_id),
                         ...
                         'target_wwn': tgt_port_wwns if tgt_port_wwns else wwns,
                         'volume_id': volume['id'],
    ...
    def _find_host_lun_id(self, hostid, lunid):
        ...
        # We sleep here to ensure the lunid take effect.
        time.sleep(2)
    ...
    def force_remove_map(self, volume):
        lun_id = volume.get('provider_location', None)
        LOG.error(_LE("force_remove_map"))
        if lun_id:
            LOG.error(_LE("lun_id=%s") % lun_id)
            if self._check_lun_exist(lun_id) is True:
                lungroup_id_list = self._get_lungroupid_by_lunid(lun_id)
                for lungroup_id in lungroup_id_list:
                    if lungroup_id is None:
    ...
    def _get_login_info(self):
        ...
        for key in ['UserName', 'UserPassword']:
            ...
            if node_text.find('!$$$') > -1:
                ...
                if key == "UserPassword":
                    logininfo[key] = crypt.decrypt(logininfo[key])
            else:
                if key == "UserPassword":
    ...
    def _get_tgt_iqn_ultrapath(self, port_ip_list):
        ...
        ret_info = []
        for target_ip in port_ip_list:
            target_iqn = None
            target_iqn = self._get_tgt_iqn_from_rest(target_ip)
            if not target_iqn:
                target_iqn = self._get_tgt_iqn(target_ip)
            if not target_iqn:
                LOG.error(_LE("Can't get iqn from target ip: %s."), target_ip)
                continue
            ret_info.append((target_iqn, target_ip))
