...
volume_opts = [
    ...
    cfg.StrOpt('tgt_conf',
               ...
               help='TGT configuration file'),
    cfg.BoolOpt('use_ultrapath_for_image_xfer',
                default=False,
                help='Do we attach/detach volumes in cinder using ultrapath '
                     'for volume to image and image to volume transfers?')
]
...
class VolumeDriver(object):
    ...
    def _attach_volume(self, context, volume, properties, remote=False):
        ...
        use_ultrapath = self.configuration.use_ultrapath_for_image_xfer
...
class ISCSIDriver(VolumeDriver):
    ...
    def do_setup(self, context):
        if CONF.iscsi_helper == 'tgtadm':
            LOG.info(_("do_setup for tgtadm..."))
            
            vol_dir = CONF.volumes_dir + '/*'
                    
            try:       
                with open(CONF.tgt_conf, 'a+') as f:
                    if vol_dir not in f.read():
                        f.write("include %s" % vol_dir)
            except Exception as e:
                LOG.error(_("do_setup exception. %s" % e))
                raise
                
            LOG.info(_("begin to restart tgt service..."))
            for pid in self.get_tgt_pids():
                os.system("sudo kill -9 %s" % pid.strip())
            tgtd_start_cmd = ('tgtd', '--iscsi', 'portal=%s:%s'\
                              %(CONF.iser_ip_address, CONF.iser_port))
            self._execute(*tgtd_start_cmd, run_as_root=True)
        
    def check_for_setup_error(self):
        if CONF.iscsi_helper == 'tgtadm':        
            LOG.info(_("check tgtd process exists."))
            flag = 3
            while not self.get_tgt_pids():
                time.sleep(1)
                flag = flag - 1
                if not flag:
                    LOG.error(_("tgtd service start fail."))
                    raise
    
            LOG.info(_("tgtd exists."))

    def get_tgt_pids(self):
        cmd = "ps -efww|grep tgtd|grep -v grep|awk '{print $2}'"
