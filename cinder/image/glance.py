...
import cinder.image.download.swift as swift_download
import cinder.image.download.uds as uds_download
...
class GlanceImageService(object):
    ...
    def download(self, context, image_id, data=None):
        ...
        if CONF.allowed_direct_url_schemes:
            ...
            if locations is None:
                locations = []
            if direct_url:
                locations.append({'url': direct_url, 'metadata': {}})
            for entry in locations:
                loc_url = entry['url']
                loc_meta = entry['metadata']
                o = urlparse.urlparse(loc_url)

                if o.scheme in CONF.allowed_direct_url_schemes:
                    if o.scheme == "file":
                        ...
                        return
                    elif o.scheme in ["uds", "uds+https"]:
                        try:
                            xfer_mod = uds_download.get_download_handler()
                            if data:
                                path = data.name
                                image_chunks = xfer_mod.download(context,
                                                                 o,
                                                                 path,
                                                                 loc_meta)
                            ...
                            LOG.exception(ex)
                    elif o.scheme in ["swift", "swift+https"]:
                        try:
                            xfer_mod = swift_download.get_download_handler()
                            if data:
                                path = data.name
                                xfer_mod.download(context,
                                                 o,
                                                 path,
                                                 loc_meta)
                                msg = _("Successfully transferred "
        ...
        if not data:
            ...
            path = data.name
            direct_io.write(path, image_chunks)
    ...
    def create(self, context, image_meta, data=None):
        ...
        RESERVED_PROPERTIES = ('__imagetype',
                               ...
                               '__platform')
        properties = {}
        properties.update(sent_service_image_meta.get('properties', {}))
        for key in properties:
            if key in RESERVED_PROPERTIES:
                reserved_properties[key] = \
                sent_service_image_meta['properties'][key]
                del sent_service_image_meta['properties'][key]
        ...
        if reserved_properties:
            ...
            cinder_context = ksclient.Client(
                        tenant_name=CONF.keystone_authtoken.admin_tenant_name,
                        username=CONF.keystone_authtoken.admin_user,
                        password=CONF.keystone_authtoken.admin_password,
                        auth_url=auth_url,
                        insecure=True)
            orig_auth_token = context.auth_token
            context.auth_token = cinder_context.auth_token
            recv_service_image_meta = self._client.call(
                context,
                'update',
                recv_service_image_meta.id,
                properties=reserved_properties)
            context.auth_token = orig_auth_token
