...
def volume_attached(context, volume_id, instance_uuid, host_name, mountpoint):
    ...
    session = get_session()
    with session.begin():
        volume_attachment_ref = volume_attachment_get(context, volume_id,
                                                      session=session)
...
def finish_volume_migration(context, src_vol_id, dest_vol_id):
    ...
    with session.begin():
        ...
        dest_volume_attachments = volume_attachment_get_used_by_volume_id(
                context, dest_vol_id, session=session)
        ...
        for attachment in dest_volume_attachments:
            msg = _('Update attachment info: %s %s (%s) =>%s') % (attachment['id'],
                    attachment['volume_id'], dest_vol_id, src_vol_id)
            LOG.debug(msg)
            setattr(attachment, 'volume_id', src_vol_id)
...
def volume_get_lock(context,volume_id,session):
...
def volume_detached(context, volume_id, attachment_id):
    ...
    with session.begin():
        ...
        for attachment in attachment_list:
            if attachment_id is None or attachment['id'] == attachment_id:
                now = timeutils.utcnow()
        ...
        volume_get_lock(context,volume_id,session)

        volume_ref = _volume_get(context, volume_id, session=session)
        if not volume_ref['volume_attachment']:
            # Hide status update from user if we're performing volume migration
            if (not volume_ref['migration_status'] and not (volume_ref['status'] == 'uploading')):
...
def _generate_paginate_query(context, session, marker, limit, sort_key,
    ...
    if filters:
        ...
        if ('changes-since' in filters and
                filters['changes-since'] is not None):
            try:
                timeStr = filters['changes-since']
                change_since_isotime = timeutils.parse_isotime(timeStr)
                changes_since = timeutils.normalize_time(change_since_isotime)
                column_attr = getattr(models.Volume, 'updated_at')
                query = query.filter(column_attr >= changes_since)
                filters.pop('changes-since')
            except AttributeError:
                log_msg = _("'update_at' column could not be found.")
                LOG.debug(log_msg)
                return None

        if ('availability-zone' in filters and
                filters['availability-zone'] is not None):
            try:
                availability_zone = filters['availability-zone']
                column_attr = getattr(models.Volume, 'availability_zone')
                query = query.filter(column_attr == availability_zone)
                filters.pop('availability-zone')
            except AttributeError:
                log_msg = _("'availability-zone' column could not be found.")
                LOG.debug(log_msg)
                return None
...
def volume_update_status_based_on_attachment(context, volume_id):
    """Update volume status based on attached instance id.

    Get volume and check if 'instance_uuid', 'attached_host' parameters
    are present in volume.
    If 'instance_uuid' and 'attached_host' is None then set volume status
    to 'available' else set volume status to 'in-use'.
...
@require_context
def backup_get_by_volume_id(context, vol_id):
    return model_query(context, models.Backup, project_only=True).\
        filter_by(volume_id=vol_id).\
        all()
