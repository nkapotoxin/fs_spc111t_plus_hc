...
def upgrade(migrate_engine):
    ...
    # add the shareable flag to the volumes table.
    volumes = Table('volumes', meta, autoload=True)
    shareable = Column('shareable', Boolean)
    volumes.create_column(shareable)
    volumes.update().values(shareable=False).execute()
    ...
    try:
        ...
        LOG.error(_("Table volume_attachment not created!"))
        raise
...
def downgrade(migrate_engine):
    ...
    volumes = Table('volumes', meta, autoload=True)
    shareable = volumes.columns.shareable
    volumes.drop_column(shareable)
    ...
    try:
        ...
        LOG.error(_("Dropping volume_attachment table failed."))
        raise
