...
class L3Scheduler(object):
    ...
    def auto_schedule_routers(self, plugin, context, host, router_ids):
        ...
        if cfg.CONF.filter_vpn_router:
            vpn_routers = [r for r in unscheduled_routers if not r.get('distributed') \
                           and r.get('name', "").startswith('public_cloud_vpn_router')]
            unscheduled_routers = [r for r in unscheduled_routers if r not in vpn_routers]
