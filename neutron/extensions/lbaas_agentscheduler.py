...
class NotLbaasAgent(agent.AgentNotFound):
    message = _("Agent %(id)s is not a Lbaas Agent")

class InvalidLbaasAgent(agent.AgentNotFound):
    message = _("Agent %(id)s is not a Lbaas Agent or has been disabled")

class PoolSchedulingFailed(exceptions.Conflict):
    message = _("Failed scheduling pool %(pool_id)s to"
                " the Lbaas Agent %(agent_id)s.")

class PoolHostedByLbaasAgent(exceptions.Conflict):
    message = _("The pool %(pool_id)s has been already hosted"
                " by the Lbaas Agent %(agent_id)s.")

class PoolNotHostedByLbaasAgent(exceptions.Conflict):
    message = _("The pool %(pool_id)s is not hosted"
...
class LbaasAgentSchedulerPluginBase(object):
    ...
    """
    @abc.abstractmethod
    def add_pool_to_lbaas_agent(self, context, id, pool_id):
        pass

    @abc.abstractmethod
    def remove_pool_from_lbaas_agent(self, context, id, pool_id):
        pass
