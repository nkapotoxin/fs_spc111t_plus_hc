...
class ExternalNetworkForFloatingIPNotFound(qexception.NotFound):
    message = _("External network %(external_network_id)s is not reachable. "
                "Therefore, cannot associate IP %(fixed_ip_address) with a Floating IP.")
...
class RouterInUseByServiceMigrationNotAllowed(qexception.InUse):
    message = _("Router %(router_id)s in use by %(service_name)s service, "
                "migration not allowed with services.")
