...
class DVRL3CannotAssignToDvrAgent(exceptions.Conflict):
    message = _("Not allowed to manually assign a %(router_type)s "
                ...
                "to another %(agent_mode)s L3 agent %(agent_id)s.")


class DVRL3CannotAssignRouterWithNoGateway(exceptions.Conflict):
    message = _("Not allowed to manually assign a %(router_type)s "
                "router %(router_id)s without a Gateway to %(agent_mode)s "
                "L3 agent %(agent_id)s.")
