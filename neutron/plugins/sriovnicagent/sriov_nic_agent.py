...
class SriovNicSwitchQoSAgent(qos_rpc.SriovQoSAgentRpcMixin):
...
class SriovNicSwitchAgent(sg_rpc.SecurityGroupAgentRpcMixin):
    ...
    def init_qos(self):
        # QoS agent support
        self.qos_agent = SriovNicSwitchQoSAgent(self.context,
                                     self.plugin_rpc,
                                     self.root_helper)        
        ...
        for device_details in devices_details_list:
            ...
            if 'port_id' in device_details:
                ...
                if device_details['qos_policies']:
                    ...
                    result = {'port':device_details}
                    if profile.get('pci_vendor_info') in qos_rpc.QOS_SUPPORTED_VENDORS_LIST:
                        self.qos_agent.qos.delete_qos_for_device(device_details['device'],
                                                                 profile.get('pci_slot'),
                                                                 **result)
            else:
