...
class L3AgentNotifyAPI(n_rpc.RpcProxy):
    ...
    def notification(self, context, method, router_ids, operation,
                     shuffle_agents):
    ...
    def routers_updated(self, context, router_ids, operation=None, data=None,
        ...
        method = 'routers_updated'
        plugin = self._get_plugin(method)
        if router_ids and plugin:
            if utils.is_extension_supported(
                    plugin, constants.L3_AGENT_SCHEDULER_EXT_ALIAS):
                adminContext = (context.is_admin and
                                context or context.elevated())
                plugin.schedule_routers(adminContext, router_ids)
            self.notification(context, method, router_ids,
