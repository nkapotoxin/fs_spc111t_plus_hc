...
class LocalZoneMapping:
    def __init__(self, zone, vif_ports=None):
        vif_ports = {}
        self.zone = zone
...
class IptablesFirewallDriver(firewall.FirewallDriver):
    ...
    def update_defer_sg_rule(self, device_ids, sg_rule):
        self.defer_sg_rules[sg_rule['id']] = {'rule': sg_rule,
                                              'devices': device_ids}

    def update_defer_sg(self, device_ids, sg_id):
        self.defer_sgs[sg_id] = device_ids
    ...
    def get_net_uuid(self, vif_id):
        for network_id, zone_mapping in self.local_zone_map.iteritems():
            if vif_id in zone_mapping.vif_ports:
                return network_id
    def remove_port_filter(self, port):
        ...
        self.iptables.apply()
        #add for ip overlap
        net_uuid = self.get_net_uuid(port['device'])
        if not self.local_zone_map.get(net_uuid):
            LOG.info(_('remove_port_filter() net_uuid %s not in local_zone_map'),
                     net_uuid)
        else:
            lzm = self.local_zone_map[net_uuid]
            lzm.vif_ports.pop(port['device'], None)

            if not lzm.vif_ports:
                LOG.debug(_("begin _reclaim_local_zone for net_uuid  (%s)"), net_uuid)
                self._reclaim_local_zone(net_uuid)
    ...
    def _setup_chains_apply(self, ports):
        ...
        self._add_raw_chain(RAW_PREROUTING)
        self.iptables.ipv4['raw'].add_rule(RAW_PREROUTING, '-j $%s' %
                                           (RAW_PREROUTING), wrap=False)
        self.iptables.ipv6['raw'].add_rule(RAW_PREROUTING, '-j $%s' %
                                           (RAW_PREROUTING), wrap=False)
    ...
    def _remove_chains_apply(self, ports):
        ...
        self._remove_raw_chain(RAW_PREROUTING)
    ...
    def _add_raw_chain_rules(self, port, direction):
        LOG.debug(_("_add_raw_chains, self.local_zone_map: %s"), self.local_zone_map)
        LOG.debug(_("port: %s"), port)
        net_uuid = port['network_id']
        if not self.local_zone_map.get(net_uuid):
            zone_id = self.available_local_zones.pop()
            self.local_zone_map[net_uuid] = LocalZoneMapping(zone_id)
        lzm = self.local_zone_map[net_uuid]
        lzm.vif_ports[port['device']] = port
        device = self._get_device_name(port)
        if direction == INGRESS_DIRECTION:
            device = ("qvm%s" % port['device'])[:LINUX_DEV_LEN]
        jump_rule = '-m physdev --physdev-in %s -j CT --zone %s' % \
                    (device, lzm.zone)
        LOG.debug(_("jump_rule: %s"), jump_rule)
        self.iptables.ipv4['raw'].add_rule(RAW_PREROUTING, jump_rule)
        self.iptables.ipv6['raw'].add_rule(RAW_PREROUTING, jump_rule)


    def _reclaim_local_zone(self, net_uuid):
        LOG.debug(_("_reclaim_local_zone, self.local_zone_map: %s"), self.local_zone_map)
        lzm = self.local_zone_map.pop(net_uuid, None)

        LOG.info(_("Reclaiming zone = %(zone_id)s from net-id = %(net_uuid)s"),
                 {'zone_id': lzm.zone,
                  'net_uuid': net_uuid})
        self.available_local_zones.add(lzm.zone)

    def _remove_raw_chain(self, chain_name):
        self.iptables.ipv4['raw'].ensure_remove_chain(chain_name)
        self.iptables.ipv6['raw'].ensure_remove_chain(chain_name)
    ...
    def filter_defer_apply_on(self):
        if not self._defer_apply:
            ...
            self.pre_sg_members = copy.deepcopy(self.sg_members)
    ...
    def _remove_unused_security_group_info(self):
        ...
        self._remove_conntrack_entries_from_cache()
    ...
    def _delete_conntrack_cmd_for_rule(self):
        cmd = ['conntrack', '-D']
        cmd_ns = []
        if self.namespace:
    ...
    def _delete_conntrack_state_by_rule(self, device_info_list):
        cmd = self._delete_conntrack_cmd_for_rule()
        net_uuid_set = set()
        for device_info in device_info_list:
            net_uuid = device_info.get('network_id')
            net_uuid_set.add(net_uuid)
        for net_id in net_uuid_set:
            lzm = self.local_zone_map[net_id]
            zone_id = lzm.zone
            if not zone_id:
                continue
            zone_cmd = ['-w', zone_id]
            try:
                utils.execute(cmd + zone_cmd, self.root_helper,
                                  check_exit_code=True,
                                  extra_ok_codes=[1])
            except RuntimeError:
                    LOG.exception(_LE("Failed deleting conntrack zone %s"), zone_id)


    def _remove_conntrack_entries_from_cache(self):
        rule_ids_to_delete = set()
        sg_ids_to_delete = set()
        device_info_list = []
        device_id_set = set()
        for rule_id, rule_info in self.defer_sg_rules.items():
            device_ids = rule_info.get('devices')
            for device_id in device_ids:
                device_id_set.add(device_id)
            rule_ids_to_delete.add(rule_id)
        for device_id in device_id_set:
            device_info = self.filtered_ports.get(device_id)
            if device_info:
                device_info_list.append(device_info)
        self._delete_conntrack_state_by_rule(device_info_list)
        for id in rule_ids_to_delete:
            self.defer_sg_rules.pop(id, None)

        for sg_id, device_ids in self.defer_sgs.items():
            if self._get_deleted_members(sg_id):
                device_info_list = []
                for device_id in device_ids:
                    device_info = self.filtered_ports.get(device_id)
                    if device_info:
                        device_info_list.append(device_info)
                self._delete_conntrack_state_by_rule(device_info_list)
                sg_ids_to_delete.add(sg_id)

        for id in sg_ids_to_delete:
            self.defer_sgs.pop(id)

    def _get_deleted_members(self, sg_id):
        deleted_members = set()
        for ether_type in [constants.IPv4, constants.IPv6]:
            deleted_members.update(self._get_deleted_sg_member_ips(sg_id, ether_type))
