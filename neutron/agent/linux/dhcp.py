...
class DhcpLocalProcess(DhcpBase):
    ...
    def check_process(self):
        if not self.active:
            LOG.warning(_('DHCP for %s is down, restarting') % self.network.id)
            self.enable()
...
class Dnsmasq(DhcpLocalProcess):
    ...
    def spawn_process(self):
        ...
        if self.network.get('provider:network_type',None) == 'vxlan':
            cmd.append('--dhcp-option-force=26,1400')
...
class DeviceManager(object):
    ...
    def setup_dhcp_port(self, network):
        ...
        for index, port in enumerate(network.ports):
            if port.id == dhcp_port.id:
                network.ports[index] = dhcp_port
                break
        else:
