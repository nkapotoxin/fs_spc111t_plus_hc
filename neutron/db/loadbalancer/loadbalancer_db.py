...
class Vip(model_base.BASEV2, models_v2.HasId, models_v2.HasTenant,
    ...
    extra_listeners = orm.relationship('Listener',
                                       backref='listener',
                                       lazy="joined",
                                       cascade='delete')
...
class LoadBalancerPluginDb(loadbalancer.LoadBalancerPluginBase,
    ...
    def _show_vip_dict(self, vip, fields=None):
        fixed_ip = (vip.port.fixed_ips or [{}])[0]
        listeners = []
        extra_listeners = vip['extra_listeners']
        for i in range(len(extra_listeners)):
            listener = {}
            listener['id'] = extra_listeners[i]['id']
            listener['status'] = extra_listeners[i]['status']
            listener['protocol'] = extra_listeners[i]['protocol']
            listener['protocol_port'] = extra_listeners[i]['protocol_port']
    ...
    def create_vip(self, context, vip):
        ...
        if cfg.CONF.lbaas_vip_create_port:
            ...
            try:
                ...
                filters = {'fixed_ips': {'subnet_id': [v['subnet_id']], 'ip_address': [v.get('address')]}}
                vip_ports = self._core_plugin.get_ports(context, filters=filters)
                if not vip_ports:
                    raise loadbalancer.VipPortNotFound(address=v.get('address'),
                                                       subnet_id=v['subnet_id'])
                vip_port = vip_ports[0]
    
                filters = {'port_id': [vip_port['id']], 'protocol_port': [v['protocol_port']]}
                vips = self.get_vips(context,filters=filters)
                if vips:
                    raise loadbalancer.VipProtocolPortInUse(address=v.get('address'),
                                                            protocol_port=v['protocol_port'])
    ...
    def delete_vip(self, context, id):
        ...
        if vip.port and cfg.CONF.lbaas_vip_create_port:  # this is a Neutron port
            self._core_plugin.delete_port(context, vip.port.id)

    def get_vip(self, context, id, fields=None):
        vip = self._get_resource(context, Vip, id)
        return self._show_vip_dict(vip, fields)
            ...
            for member, stats in data.get('members', {}).items():
                ...
                if stats_status:
                    try: 
                        ...
                        LOG.exception(_('Member %s update status crushed'), member)
