...
class NeutronDbPluginV2(neutron_plugin_base_v2.NeutronPluginBaseV2,
    ...
    @staticmethod
    def _generate_mac(context, network_id, mac_global = False):
    ...
    @staticmethod
    def _check_unique_mac(context, network_id, mac_address, mac_global = False):
        ...
        if not mac_global:
            ...
            try:
                mac_qry.filter_by(mac_address=mac_address).one()
            except exc.NoResultFound:
                return True
            except exc.MultipleResultsFound:
                LOG.error(_("Multiple ports have mac address %s"), mac_address)
                return False
    ...
    def _rebuild_availability_ranges(context, subnets):
        ...
        """
        context.session.execute("LOCK TABLE ipavailabilityranges IN EXCLUSIVE MODE;")
        ...
        for fixed in fixed_ips:
            ...
            if 'ip_address' in fixed:
                ...
                if (not found and
                    ...
                    raise n_exc.InvalidInput(error_message=msg)
                if (ipv6_utils.is_auto_address_subnet(subnet) and device_owner not in
    ...
    def create_subnet(self, context, subnet):
        ...
        if s['gateway_ip'] is attributes.ATTR_NOT_SPECIFIED:
            if s['ip_version'] == 6 and ipv6_utils.is_auto_address_subnet(s):
                s['gateway_ip'] = None
            else:
    ...
    def delete_subnet(self, context, id):
        with context.session.begin(subtransactions=True):
            ...
            # via SLAAC
            is_ipv6_slaac_subnet = ipv6_utils.is_auto_address_subnet(subnet)
            for a in allocated:
    ...
    def create_port(self, context, port):
        ...
        with context.session.begin(subtransactions=True):
            ...
            # network
            mac_global = False
            if cfg.CONF.enable_trunkport and p.get('trunkport:type') != 'subport':
                mac_global = True
