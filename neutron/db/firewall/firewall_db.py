...
class FirewallRule(model_base.BASEV2, models_v2.HasId, models_v2.HasTenant):
    ...
    rule_profile = sa.Column(sa.String(1024))
...
class Firewall_db_mixin(firewall.FirewallPluginBase, base_db.CommonDbMixin):
    ...
    def _make_firewall_rule_dict(self, firewall_rule, fields=None):
        ...
        res = {'id': firewall_rule['id'],
               ...
               'mode': firewall_rule['mode'],
               'rule_profile': firewall_rule['rule_profile']}
        return self._fields(res, fields)
    ...
    def _get_min_max_ports_from_range(self, port_range):
        ...
        if int(min_port) > int(max_port):
            LOG.error(_("firewall rule, the min_port: %s is larger than max_port:%s." % (min_port, max_port)))
    ...
    def update_firewall_rule(self, context, id, firewall_rule):
        ...
        with context.session.begin(subtransactions=True):
            ...
            if ( "protocol" in fwr ) and (fwr["protocol"] in ["icmp", "ICMP"]):
                fwr['source_port_range_min'] = None
                fwr['source_port_range_max'] = None   
                fwr['destination_port_range_min'] = None
                fwr['destination_port_range_max'] = None
