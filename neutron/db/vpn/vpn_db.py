...
class VPNService(model_base.BASEV2, models_v2.HasId, models_v2.HasTenant):
    ...
    name = sa.Column(sa.String(255))
    description = sa.Column(sa.String(4 * 1024))
    status = sa.Column(sa.String(16), nullable=False)
    admin_state_up = sa.Column(sa.Boolean(), nullable=False)
...
class VPNPluginDb(vpnaas.VPNPluginBase, base_db.CommonDbMixin):
    ...
    def create_ipsec_site_connection(self, context, ipsec_site_connection):
        ...
        # pre_shared_key should be encrypted before stored in db
        if "psk" in ipsec_sitecon:
            ipsec_sitecon["psk"] = crypt.encrypt(ipsec_sitecon.get("psk"))
    ...
    def _update_connection_status(self, context, conn_id, new_status,
        ...
        try:
            ...
            return
        if conn_db.status == constants.PENDING_DELETE:
            return
    ...
    def create_vpnservice(self, context, vpnservice):
        ...
        validator = self._get_validator()
        with context.session.begin(subtransactions=True):
            validator.validate_vpnservice_ngfw(context, vpns)
...
class VPNPluginRpcDbMixin(VPNPluginDb,
                          ...
                          l3_agent_db.L3AgentSchedulerDbMixin,
    ...
    def delete_connections(self, context, conn_ids):
        LOG.debug(_("delete connections in db: %s" % conn_ids))
        for conn_id in conn_ids:
            super(VPNPluginRpcDbMixin, self).delete_ipsec_site_connection(
                context, conn_id)

    def update_router(self, context, id, router):
        r = router['router']
        distributed = r.get('distributed', True)
        r['distributed'] = distributed
        router_db = super(VPNPluginRpcDbMixin, self).update_router(context, id, router)
        LOG.debug("Update router finished, the router is %s" % router_db)
        self.notify_router_updated(context, id, None, None)

    def get_router_info(self, context, id):
        router = super(VPNPluginRpcDbMixin, self).get_router(context, id, None)
        self._make_dict(context, router)
        return router

    def get_networks(self, context, filters=None, fields=None):
        return self._core_plugin.get_networks(context, filters, fields)

    def get_agent_by_router_id(self,context,router_id):
        agent = self.list_l3_agents_hosting_router(context,router_id)
        return agent

    def _make_dict(self, context, router):
        if not router['gw_port_id']:
            return
        ext_subnet_id = router['external_gateway_info']['external_fixed_ips'][0]['subnet_id']
        if not ext_subnet_id:
            return

        subnet_info = self._core_plugin.get_subnet(context, ext_subnet_id)
        if not subnet_info:
            return

        router['external_gateway_info']['subnet'] = subnet_info

    def _update_vpnservice(self, context, vpnservice):
        vpnservice['local_subnets'] = []
        vpnservice['virtual_ip'] = None
        vpnservice['tenant_router_info'] = {}
        try:
            ...
            for subnet_id in subnets:
                subnet_info = self._core_plugin.get_subnet(context, subnet_id)
                if subnet_info:
                    vpnservice['local_subnets'].append(subnet_info)
            #append tenant router info
            tenant_router_info = self.get_router_info(context, tenant_router_id)
            if tenant_router_info:
                vpnservice['tenant_router_info'] = tenant_router_info
            LOG.debug("vpnservice: update router success")
        except:
            LOG.error("update router for vpnservice error")
