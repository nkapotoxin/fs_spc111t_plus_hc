...
class L3_NAT_dbonly_mixin(l3.RouterPluginBase):
    ...
    def _create_router_gw_port(self, context, router, network_id, ext_ips):
        if ext_ips and len(ext_ips) > 1:
                msg = _("Routers currently support only 1 external IP")
                raise n_exc.BadRequest(resource='router', msg=msg)
    ...
    def _delete_current_gw_port(self, context, router_id, router, new_network,
        ...
        if (router.gw_port and
            (is_gw_port_attached_to_existing_network or ext_ip_change)):
    ...
    def _check_for_external_ip_change(self, context, gw_port, ext_ips):
        ...
        return ip_address_change or subnet_change
    ...
    def _add_interface_by_subnet(self, context, router, subnet_id, owner):
        ...
        if (not subnet['gateway_ip']
            and not ipv6_utils.is_auto_address_subnet(subnet)):
    ...
    def _get_gw_ports_for_network(self, context, network_id):
        router_intf_qry = context.session.query(RouterPort)
        router_intf_qry = router_intf_qry.join(models_v2.Port)
        return router_intf_qry.filter(
            models_v2.Port.network_id == network_id,
    ...
    def _get_router_for_floatingip(self, context, internal_port,
        ...
        #whether the router can create fip without gateway
        if router_port and cfg.CONF.create_fip_without_gw:
            return router_port.router.id

        if router_port and router_port.router.gw_port:
            if router_port.router.gw_port.get('network_id') == external_network_id:
                return router_port.router.id
    ...
    def _check_and_get_fip_router(self, context, fip, floating_network_id):
        routerport_qry = self._get_gw_ports_for_network(context, floating_network_id)
        router_port = routerport_qry.first()
        if router_port and router_port.router.gw_port:
            return router_port.router.id

        raise l3.ExternalNetworkForFloatingIPNotFound(
    ...
    def _check_and_get_fip_assoc(self, context, fip, floatingip_db):
        ...
        if (('fixed_ip_address' in fip and fip['fixed_ip_address']) and
            not ('port_id' in fip and fip['port_id']) and not cfg.CONF.fip_only_specify_ip_allowed):
            msg = _("fixed_ip_address cannot be specified without a port_id")
            raise n_exc.BadRequest(resource='floatingip', msg=msg)
        elif (('fixed_ip_address' in fip and fip['fixed_ip_address']) and
            not ('port_id' in fip and fip['port_id']) and cfg.CONF.fip_only_specify_ip_allowed):
            internal_ip_address = fip['fixed_ip_address']
            filters = {'network': [floatingip_db['floating_network_id']], \
                       'fixed_ips': {'ip_address': [floatingip_db['floating_ip_address']]}}
            floating_port = self._core_plugin.get_ports(context, filters=filters) or []
            if len(floating_port) < 1:
                msg = _("can not match a floatingip port")
                raise n_exc.BadRequest(resource='floatingip', msg=msg)
            cas_router_id = floating_port[0].get('name')

            router_filters = {'name': ['router@'+ cas_router_id]}
            router = self.get_routers(context, filters=router_filters) or []
            if len(router) < 1:
                msg = _("can not match the router for the floatingip port")
                raise n_exc.BadRequest(resource='floatingip', msg=msg)
            router_id = router[0].get('id')

            if cfg.CONF.nexthop_over_tunneling == 'gre':
                router = self._get_sync_routers(context,
                                             router_ids=[router_id])
                dest_cidrs = [ext['destination']
                            for ext in router[0]['routes']]
                if not netaddr.all_matching_cidrs(internal_ip_address, dest_cidrs):
                    msg = _("fixed_ip_address cannot be specified without a unreach nexthop")
                    raise n_exc.BadRequest(resource='floatingip', msg=msg)
        elif 'port_id' in fip and fip['port_id']:
            ...
            try:
                ...
                if not cfg.CONF.multi_fip_per_port_allowed:
    ...
    def create_floatingip(self, context, floatingip,
        ...
        with context.session.begin(subtransactions=True):
            ...
            port = {'tenant_id': '',  # tenant intentionally not set
                    ...
                    'name': ''}
            flag = True
            #get external_relay_netword id and judge whether its true ext_net
            if cfg.CONF.external_relay_network_name  != 'default':
                filters = {'name':[cfg.CONF.external_relay_network_name]}
                fields = ['id']
                external_relay_network = self._core_plugin.get_networks(
                                                    context, filters,fields)
                external_relay_network_id  = external_relay_network[0].get('id',None)
                if f_net_id == external_relay_network_id:
                    flag = False
            
            #create fip with vm port
            if fip.get('port_id', None) and not flag:
                internal_port = self._core_plugin.get_port(
                                       context.elevated(), fip.get('port_id', None))
                #get vm port region_name 
                port_host_id = internal_port.get('binding:host_id', None)
                
                #get internal_relay_network
                filters = {'name':[cfg.CONF.internal_relay_network_name]}
                fields = ['id']
                internal_relay_network = self._core_plugin.get_networks(
                                                context, filters,fields)
                
                #get subnet by port region_name
                filters = {'network_id': [internal_relay_network[0].get('id',None)],
                           'name': [port_host_id]}
                
                region_subnet = self._core_plugin.get_subnets(
                                        context, filters, fields)[0].get('id',None)
                
                #create internal_relay_port:
                internal_relay_port = self._core_plugin.create_port(
                                            context.elevated(),{
                    'port':
                    {'tenant_id': internal_port.get('tenant_id', None),  # tenant id is vm port tenant_id
                     'network_id': internal_relay_network[0].get('id',None),
                     'mac_address': attributes.ATTR_NOT_SPECIFIED,
                     'fixed_ips': [{'subnet_id' : region_subnet}],
                     'admin_state_up': True,
                     'device_id': '',
                     'device_owner': '',
                     'name': ''}})
                port['fixed_ips'] = [
                    {'ip_address': internal_relay_port.get('fixed_ips')[0].get('ip_address')}]
