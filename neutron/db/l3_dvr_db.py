...
router_distributed_opts = [
    ...
    cfg.BoolOpt('disable_snat_interface',
                default=False,
                help=_("When this config is True and router's enable_snat is False,"
                       "the snat port will not be created. Only admin can override.")),
    cfg.BoolOpt('fip_gw_port_fixed',
                default=False,
                help=_("Fixed the floatingip gateway not to be delete")),
]
...
class L3_NAT_with_dvr_db_mixin(l3_db.L3_NAT_db_mixin,
    ...
    def check_router_has_no_firewall(self, context, router_db):
        ...
        if fwaas_service:
            ...
            if tenant_firewalls:
                raise l3.RouterInUseByServiceMigrationNotAllowed(
    ...
    def check_router_has_no_vpnaas(self, context, router_db):
        ...
        vpn_plugin = manager.NeutronManager.get_service_plugins().get(
            constants.VPN)
        if vpn_plugin:
            vpnservices = vpn_plugin.get_vpnservices(
                context, filters={'router_id': [router_db['id']]})
            if vpnservices:
                raise l3.RouterInUseByServiceMigrationNotAllowed(
                    router_id=router_db['id'],
                    service_name="VPN")
    ...
    def _delete_current_gw_port(self, context, router_id, router, new_network,
        ...
        if router.extra_attributes.distributed and not new_network:
    ...
    def get_routers(self, context, filters=None, fields=None,
        ...
        query = self._get_routers_query(context, filters=filters,
    ...
    def _update_fip_assoc(self, context, fip, floatingip_db, external_port):
        ...
        unused_agent_port = fip.get('port_id')
        admin_ctx = context.elevated()
        if unused_agent_port is None and floatingip_db['fixed_port_id']:
    ...
    def clear_unused_fip_agent_gw_port(
        ...
        if fip_hostid and self.check_fips_availability_on_host(
            context, fip_id, fip_hostid, net_id):
            if not cfg.CONF.fip_gw_port_fixed:
                LOG.debug('Deleting the Agent GW Port on host: %s', fip_hostid)
                self.delete_floatingip_agent_gateway_port(context, fip_hostid, net_id)
    ...
    def _process_floating_ips(self, context, routers_dict, floating_ips):
        for floating_ip in floating_ips:
            ...
            if router:
                ...
                if router['distributed'] and floating_ip['port_id']:
    ...
    def get_vm_port_hostid(self, context, port_id, port=None):
        ...
        try:
            ...
            LOG.warn(_("Attempted to get hostid of port %s which was deleted"), port_id)
            return None
    ...
    def delete_floatingip_agent_gateway_port(self, context, host_id, net_id):
    ...
    def add_csnat_router_interface_port(
        ...
        if cfg.CONF.disable_snat_interface and not router.get('enable_snat', True):
            return []
    ...
    def create_snat_intf_ports_if_not_exists(self, context, router):
        ...
        """
        #Disable snat interface for hw-public-cloud
        if cfg.CONF.disable_snat_interface and not router.get('enable_snat', True):
            return []
    ...
    def dvr_vmarp_table_update(self, context, port_id, action):
        ...
        filters = {'fixed_ips': {'subnet_id': [subnet]}, 'device_owner': [DEVICE_OWNER_DVR_INTERFACE]}
    ...
    def dvr_remote_port_vmarp_table_update(self, context, port_id, action):
        """Notify the L3 agent of VM ARP table changes.
