...
AGENTS_SCHEDULER_OPTS = [
    ...
    cfg.ListOpt('dhcp_network_filter',
                default=[],
                help=_('Only scheduler the name of network in filter list.'))
]
...
class AgentSchedulerDbMixin(agents_db.AgentDbMixin):
    ...
    def is_eligible_agent(active, agent):
        if active is None:
            ...
            agent_conf = agents_db.AgentDbMixin.get_dvr_configuration_dict(agent)
            agent_mode = agent_conf.get('agent_mode')
            if agent_mode == 'dvr':
                LOG.debug("The Agent Mode is %s", agent_mode)
                return True
...
class DhcpAgentSchedulerDbMixin(dhcpagentscheduler
    ...
    def distributed_add_network_to_host(self, context, network_id, host_id):
        if not host_id:
            return False
        LOG.debug('distributed_add_network_to_host_network_id:%s,host_id:%s', network_id, host_id)
        dhcp_agent = self.get_enabled_agent_on_host(context, constants.AGENT_TYPE_DHCP, host_id)
        if dhcp_agent:
            with context.session.begin(subtransactions=True):
                query = context.session.query(NetworkDhcpAgentBinding)
                binding = query.filter(
                    NetworkDhcpAgentBinding.network_id == network_id,
                    NetworkDhcpAgentBinding.dhcp_agent_id == dhcp_agent.id).all()
                if not binding:
                    self.add_network_to_dhcp_agent(context, dhcp_agent.id, network_id)
            return True
        else:
            return False

    def distributed_remove_network_from_host(self, context, network_id, host_id):
        LOG.debug('distributed_remove_network_from_host_network_id:%s,host_id:%s', network_id, host_id)
        dhcp_agent = self.get_enabled_agent_on_host(context, constants.AGENT_TYPE_DHCP, host_id)
        if dhcp_agent:
            try:
                self.remove_network_from_dhcp_agent(context, dhcp_agent.id, network_id)
            except dhcpagentscheduler.NetworkNotHostedByDhcpAgent:
                LOG.warn('remove no exist binding, network_id:%s, host_id:%s', network_id, host_id)
