...
class VPNDriverPlugin(VPNPlugin, vpn_db.VPNPluginRpcDbMixin):
    ...
    def __init__(self):
        ...
        self.pluginutils = ngfw_plugin.NGFWPluginUtils()

    def _get_driver_for_vpnservice(self, vpnservice):
        return self.ipsec_driver

    def _check_vaild_ipsec_site_connection(self,context,ipsec_site_connection):
        LOG.info(_('ipsec_site_connection is :%s' %
                   ipsec_site_connection.get("id")))
        ret = self.pluginutils.get_proper_agent_by_ip('vpn',ipsec_site_connection['description'])
        if (None, None) == ret:
            LOG.error(_('external_ip:%s is invalid public ip' % ipsec_site_connection['description']))
            raise vpnaas.IPsecSiteConnectionInvalid(ipsec_site_connection=ipsec_site_connection)
    ...
    def create_ipsec_site_connection(self, context, ipsec_site_connection):
        self._check_vaild_ipsec_site_connection(context,ipsec_site_connection['ipsec_site_connection'])
    ...
    def delete_ipsec_site_connection(self, context, ipsec_conn_id):
        ...
        self._check_vaild_ipsec_site_connection(context,ipsec_site_connection)
        conn_id = ipsec_site_connection.get("id")
        super(VPNDriverPlugin, self).update_ipsec_site_conn_status(context,
                                                            conn_id,
                                                            "PENDING_DELETE")
        driver = self._get_driver_for_ipsec_site_connection(
            context, ipsec_site_connection)
    ...
    def update_ipsec_site_connection(
        ...
        old_ipsec_site_connection = self.get_ipsec_site_connection(
            context, ipsec_conn_id)
        self._check_vaild_ipsec_site_connection(context,old_ipsec_site_connection)
        ipsec_site_connection = super(
