...
class IPsecVpnDriverCallBack(n_rpc.RpcCallback):
    ...
    def get_vpn_services_on_host(self, context, host=None):
        ...
        for vpnservice in vpnservices:
            vpnservice_dict = self.driver._make_vpnservice_dict(vpnservice)
            vpnservice_updated = plugin._update_vpnservice(context,vpnservice_dict)
            vpn.append(vpnservice_updated)
        return vpn

    def delete_connections(self, context, conn_ids):
        plugin = self.driver.service_plugin
        plugin.delete_connections(context, conn_ids)
    ...
    def update_router(self, context, id, router):
        plugin = self.driver.service_plugin
        plugin.update_router(context, id, router)

    def get_router(self, context, id):
        plugin = self.driver.service_plugin
        return plugin.get_router_info(context, id)

    def get_networks(self, context, filters=None, fields=None):
        plugin = self.driver.service_plugin
        return plugin.get_networks(context, filters, fields)

    def get_agent_by_router_id(self, context, router_id):
        plugin = self.driver.service_plugin
        return plugin.get_agent_by_router_id(context, router_id)
