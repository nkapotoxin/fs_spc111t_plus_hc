...
from neutron.services.loadbalancer.drivers.haproxy_integration \
    import integration_driver
...
class LbaasAgentManager(n_rpc.RpcCallback, periodic_task.PeriodicTasks):
    ...
    def __init__(self, conf):
        ...
        self._load_drivers()
        self.haproxy_int_driver = self.device_drivers.get(integration_driver.DRIVER_NAME)
    ...
    def _report_state(self):
        try:
            ...
            if self.haproxy_int_driver:
                self.agent_state['configurations'].update(
                    self.haproxy_int_driver.get_proxy_info())
    ...
    def get_stats(self, pool_id):       
        driver = self.device_drivers[self.instance_mapping[pool_id]]
        local_stats = driver.get_stats(pool_id)
  
        if local_stats and driver.get_name() != integration_driver.DRIVER_NAME:
            driver.stats_cache.set_stats(pool_id, local_stats)
            
            server_stats = self.plugin_rpc.get_pool_stats(pool_id)
            local_stats[lb_const.STATS_IN_BYTES] += server_stats[lb_const.STATS_IN_BYTES]
            local_stats[lb_const.STATS_OUT_BYTES] += server_stats[lb_const.STATS_OUT_BYTES]        
   
        return local_stats
    
    @periodic_task.periodic_task(spacing=6)
    def send_haproxy_config_data(self, context):
        if self.haproxy_int_driver:
            self.haproxy_int_driver.send_haproxy_config_data()

    @periodic_task.periodic_task(spacing=30)
    def process_static_routes(self, context):
        if self.haproxy_int_driver:
            self.haproxy_int_driver.process_static_routes()
    ...
    @periodic_task.periodic_task
    def check_driver(self, context):  
        ready_logical_devices = set(self.plugin_rpc.get_ready_devices())
        
        for pool_id, driver_name in self.instance_mapping.items():
            driver = self.device_drivers[driver_name]
            
            if pool_id in ready_logical_devices and not driver.exists(pool_id):
                try: 
                    logical_config = self.plugin_rpc.get_logical_device(pool_id)         
                
                    flag = driver.check_process(pool_id)
                    if flag:
                        driver.update(logical_config)
                    else:
                        driver.create(logical_config)
                    driver.stats_cache.put(pool_id)
                except Exception:
                    LOG.exception(_("Driver is down of pool: %s"), pool_id)
    ...
    def create_listener(self, context, listener, pool_id):
        driver = self._get_driver(pool_id)
        try:
            driver.create_listener(pool_id)
        except Exception:
            self._handle_failed_driver_call('create', 'listener', listener['id'],
                                            driver.get_name())
        else:
            self.plugin_rpc.update_status('listener', listener['id'], constants.ACTIVE)
    
    def delete_listener(self, context, pool_id):
        driver = self._get_driver(pool_id)
        driver.delete_listener(pool_id)
