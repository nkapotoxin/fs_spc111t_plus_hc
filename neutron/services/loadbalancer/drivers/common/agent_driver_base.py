...
from neutron.db.loadbalancer import loadbalancer_db_mixin
...
class LoadBalancerCallbacks(n_rpc.RpcCallback):
    ...
    def get_logical_device(self, context, pool_id=None):
        with context.session.begin(subtransactions=True):
            ...
            if pool.vip:
                ...
                retval['listeners'] = [
                    self.plugin._make_listener_dict(l)
                    for l in pool.vip.extra_listeners 
                ]
    ...
    def pool_deployed(self, context, pool_id):
        with context.session.begin(subtransactions=True):
            ...
            if pool.vip:
                for l in pool.vip.extra_listeners:
                    if l.status in constants.ACTIVE_PENDING_STATUSES:
                        l.status = constants.ACTIVE
    ...
    def update_status(self, context, obj_type, obj_id, status):
        model_mapping = {
            ...
            'listener':loadbalancer_db_mixin.Listener
        }
    ...
    def get_pool_stats(self, context, pool_id):
        with context.session.begin(subtransactions=True):
            qry = context.session.query(loadbalancer_db.PoolStatistics)
            qry = qry.filter_by(pool_id=pool_id)
            try:
                stats = qry.one()
            except Exception:
                return {lb_const.STATS_IN_BYTES: 0,
                        lb_const.STATS_OUT_BYTES: 0}
                
            retval = {}
            
            retval[lb_const.STATS_IN_BYTES] = stats[lb_const.STATS_IN_BYTES]
            retval[lb_const.STATS_OUT_BYTES] = stats[lb_const.STATS_OUT_BYTES]
            
            return retval
    
    def get_proxy_info(self, context, host):
        def get_lbaas_agent():
            with context.session.begin(subtransactions=True):
                query = context.session.query(agents_db.Agent)
                query = query.filter(agents_db.Agent.host == host,
                                     agents_db.Agent.agent_type == q_const.AGENT_TYPE_LOADBALANCER)
                return query.first()
        
        agent = get_lbaas_agent()
        if agent:
            configuration = jsonutils.loads(agent.configurations)
            external_networks = configuration.get('external_networks', [])
            removed_external_networks = configuration.get('removed_external_networks', [])
            return configuration.get('proxy_ips', []), \
                   external_networks + removed_external_networks
...
class LoadBalancerAgentApi(n_rpc.RpcProxy):
    ...
    def create_listener(self, context, listener, pool_id, host):
        return self._cast(context, 'create_listener', {'listener': listener,
                                                       'pool_id': pool_id}, host)

    def delete_listener(self, context, pool_id, host):
        return self._cast(context, 'delete_listener',{'pool_id': pool_id} , host) 
...
class AgentDriverBase(abstract_driver.LoadBalancerAbstractDriver):
    ...
    def update_vip(self, context, old_vip, vip):
        if old_vip['pool_id'] != vip['pool_id']:
            ...
            if old_pool_agent:
                self.agent_rpc.delete_vip(context, old_vip,
    ...
    def add_pool_to_host(self, context, pool, host):
        self.agent_rpc.create_pool(context, pool, host, self.device_driver)

    def remove_pool_from_host(self, context, pool, host):
        self.agent_rpc.delete_pool(context, pool, host)  
          
    def create_vip_listener(self, context, listener, pool_id):
        agent = self.get_pool_agent(context, pool_id)
        self.agent_rpc.create_listener(context, listener, pool_id, agent['host'])
    
    def delete_vip_listener(self, context, vip_id, pool_id, id):
        agent = self.get_pool_agent(context, pool_id)
        self.plugin._delete_db_listener(context, id, vip_id)
        self.agent_rpc.delete_listener(context, pool_id, agent['host'])
